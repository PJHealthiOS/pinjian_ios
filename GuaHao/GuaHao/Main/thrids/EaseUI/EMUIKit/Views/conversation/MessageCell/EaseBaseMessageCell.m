...
- (void)setModel:(id<IMessageModel>)model
{
    ...
    if(self.pinjian){
        if(self.model.isSender){
            _nameLabel.text = self.pinjian[@"senderName"];
            [self.avatarView sd_setImageWithURL:[NSURL URLWithString:self.pinjian[@"senderAvatar"]] placeholderImage:model.avatarImage];
        }else{
            _nameLabel.text = self.pinjian[@"receiverName"];
            self.avatarView.image = [UIImage imageNamed:self.pinjian[@"receiverAvatar"]];
        }
    }
    if (self.model.isSender) {
        _hasRead.hidden = YES;
