    ...
    if (messages.count>0) {
       ...
       NSString * orderSerialNo = [self getorderSerialNo:vo.extraData];
        ...
        if (vo.type.intValue == 1) {
            ...
            [self.navigationController pushViewController:view animated:YES];
        }else if(vo.type.intValue == 16){
            [self setRead:vo.id];
            GHAcceptDetailViewController* view = [GHViewControllerLoader GHAcceptDetailViewController];
            view.isNormal = NO;
            view.serialNo = orderSerialNo;
            [self.navigationController pushViewController:view animated:YES];
        }else if(vo.type.intValue == 21||vo.type.intValue == 22||vo.type.intValue == 23||vo.type.intValue == 24||vo.type.intValue == 25||vo.type.intValue == 26||vo.type.intValue == 161){
            [self setRead:vo.id];
