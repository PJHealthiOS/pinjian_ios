...
-(BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType{
    ...
    if ([requestString hasSuffix:@"drawShareAction"]) {///抽奖分享
        [self shareClick];;
        return NO;
    }
    if ([requestString hasSuffix:@"myDiscountPage"]) {///优惠券
        [self.navigationController pushViewController:[GHViewControllerLoader GHDisdcountListViewController] animated:YES];
        return NO;
    }
    return YES;
}
