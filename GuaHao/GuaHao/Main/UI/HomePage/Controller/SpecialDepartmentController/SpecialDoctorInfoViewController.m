...
-(void)remarkDoctor{
    ...
    self.rightItemButton.frame = CGRectMake(0, 0, 30, 30);
    [self.rightItemButton setImage:[UIImage imageNamed:@"doctor_collection_up.png"] forState:UIControlStateNormal];
    [self.rightItemButton setImage:[UIImage imageNamed:@"doctor_collection_down.png"] forState:UIControlStateSelected];
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithCustomView:self.rightItemButton];
    [self.rightItemButton addTarget:self action:@selector(remarlAction:) forControlEvents:UIControlEventTouchUpInside];
    
}
- (void)remarlAction:(id)sender {
...
-(void)reloadPage{
    ...
    self.pospositionLabel.text = expertVO.title;
    self.amountLabel.text = [NSString stringWithFormat:@"预约量%@",expertVO.outpatientCount];
    self.departmentLabel.text = expertVO.departmentName;
...
- (IBAction)goodButtonAction:(UIButton *)sender {
    ...
    if (!sender.selected) {
        self.goodViewHeight.constant = 100;
    }else{
        self.goodViewHeight.constant = 80 + [self boundingRectWithSize:CGSizeMake(SCREEN_WIDTH - 40, 0) str:expertVO.expert fount:14].height;
    }
}
...
- (IBAction)experianceButtonAction:(UIButton *)sender {
    ...
    if (!sender.selected) {
        ...
        self.experienceViewHeight.constant = 80 + [self boundingRectWithSize:CGSizeMake(SCREEN_WIDTH - 40, 0) str:expertVO.resume fount:14].height;
    }
