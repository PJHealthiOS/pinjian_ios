...
@interface CreateOrderNormalViewController ()<UITableViewDelegate,UITableViewDataSource,ConditionRemarksDelegate,CertificationDelegate,AddPatientDelegate,WriteIDcardViewDelegate,OrderSureDelegate,OrderReviseDelegate,TipSureViewDelegate,FixPhoneDelegate,UIAlertViewDelegate>{
...
-(void)updateSource{
    ...
    NSArray *hospitalArr = @[@{@"type":@"order_hospital.png",@"value":@"请选择就诊医院"},@{@"type":@"order_department.png",@"value":@"请选择就诊科室"},@{@"type":@"order_date.png",@"value":@"请选择时间"},@{@"type": @"order_warning.png",@"value":[_level isEqualToString:@"0"] ? @"该时间为品简专员为您挂号的时间。挂号成功后，请于挂号当日到院，联系专员就诊。" :@"该时间为预约就诊时间。预约成功后，请于挂号当日到院，挂号窗口挂号就诊 ！"}];
...
//    if ([[self getLabelValueWithSection:0 row:2] isEqualToString:@"请选择挂号方式"]) {
//        [self inputToast:@"请选择就诊类型！"];
//        return;
//    }
    if (!_hospital) {
...
-(void)popSure
{
    ...
    [sureView setName:_patientVO.name content:[self getLabelValueWithSection:0 row:2]];
...
-(void)fixPhoneDelegate:(BOOL) isSure phone:(NSString*) phone
{
    ...
    if (phone && phone.length != 0) {
         [self updateTableViewWithSection:1 row:1 key:@"value" newValue:phone];
    }
}
